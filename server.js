    ...
    io.on('connection', socket => {
        ...
        socket.on('join-session', async data => {
            ...
            socket.emit('session-state', {
                ...
                hostId: session.hostId,
                current: participantWithPermissions,
                isHost,
                canEdit,
            });
        ...
        //     console.log('=== TOGGLE PARTICIPANT EDIT START ===', data);

        //     const { roomCode, participantId, canEdit } = data;
        //     const session = sessionStore.get(roomCode);

        //     if (!session) {
        //         console.warn(`[DEBUG] No session found for room ${roomCode}`);
        //         return;
        //     }

        //     if (!socket.participant?.isHost) {
        //         console.warn(`[DEBUG] User is not host:`, socket.participant);
        //         socket.emit('error', { message: 'Only host can change participant permissions' });
        //         return;
        //     }

        //     // Find participant by `id` field instead of key
        //     const participantKey = Object.keys(session.participants).find(
        //         key => session.participants[key].id === participantId
        //     );

        //     if (!participantKey) {
        //         console.warn(`[DEBUG] No matching participant found for id ${participantId}`);
        //         console.log('Available participants:', Object.values(session.participants));
        //         return;
        //     }

        //     // Update permission
        //     session.participants[participantKey].canEdit = canEdit;
        //     sessionStore.set(roomCode, session);
        ...
        //     );
        // });
        socket.on('toggle-participant-edit', ({ roomCode, participantId, canEdit }) => {
            const session = sessionStore.get(roomCode);

            if (!session || !socket.participant?.isHost) {
                socket.emit('error', { message: 'Only host can change permissions' });
                return;
            }

            // Find the target participant by custom ID
            const targetSocketId = Object.keys(session.participants).find(
                id => session.participants[id].id === participantId
            );

            if (!targetSocketId) {
                console.log('[DEBUG] Participant not found:', participantId);
                return;
            }

            // Update participant permission in session
            session.participants[targetSocketId].canEdit = canEdit;
            sessionStore.set(roomCode, session);

            // Emit only to the specific participant
            io.to(targetSocketId).emit('participant-edit-changed', {
                participantId,
                canEdit,
                participants: Object.values(session.participants),
            });
            io.to(roomCode).emit('participant-edit-changed-for-all', {
                participantId,
                canEdit,
                participants: Object.values(session.participants),
            });
            console.log(`[BACKEND] Permission ${canEdit ? 'granted' : 'revoked'} for ${participantId}`);
        });
